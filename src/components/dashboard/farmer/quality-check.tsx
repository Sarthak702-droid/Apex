  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      toast({
        ...
        description: 'AI has analyzed your produce.',
      });
    } catch (error) {
      ...
      <Card>
        <CardHeader>
          <CardTitle className="font-headline flex items-center gap-2">
            ...
            AI Produce Quality Check
          </CardTitle>
          <CardDescription>
            Upload an image of your crop to get an instant AI-powered quality analysis before you sell.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormField control={form.control} name="produceName" render={({ field }) => (
                <FormItem><FormLabel>Produce Name</FormLabel><FormControl><Input placeholder="e.g., Soybeans" {...field} /></FormControl><FormMessage /></FormItem>
              )} />
                      ...
                      >
                        ...
                        {preview ? (
                          <div className='relative w-full h-40'>
                             <Image src={preview} alt="Produce preview" fill={true} objectFit="contain" />
                          </div>
                        ) : (
